...
export class AccionReportes implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.accionService.getComparativaMensual().subscribe({
      next: (data) => {
        this.datos = data;
        this.generarGrafico();
      },
  ...
  private generarGrafico(): void {
    const meses = [...new Set(this.datos.map((x) => x.mes))];
    const autoridades = [...new Set(this.datos.map((x) => x.autoridad))];
    ...
    const datasets = autoridades.map((autoridad) => {
      const valores = meses.map((mes) => {
        const registro = this.datos.find((x) => x.mes === mes && x.autoridad === autoridad);
